function [psi_array] = prop_MH(psi0, field, H0, mu, tau, opt)
% Propagates states using the Al-Mohy Higham algorithm.
arguments
...
switch opt.direction
	case "fwd"
		...
		psi_array(:,:,1) = psi0;
		psi_array = prop_MH_fwd(psi_array, field, H0, mu, tau);
	case "bwd"
		% initial states are row vectors
		psi_array = complex(zeros(Nrow,Ncol,Ntau+1));
		psi_array(:,:,Ntau+1) = psi0;
		psi_array = prop_MH_bwd(psi_array, field, H0, mu, tau);
end
% done
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [psi_array] = prop_MH_fwd(psi_array, field, H0, mu, tau)
...
function [psi_array] = prop_MH_bwd(psi_array, field, H0, mu, tau)
